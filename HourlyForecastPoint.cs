...
namespace Xcalibur.Weather.Models
{
    ...
    public class HourlyForecastPoint : BaseForecastPoint
    {
        ...
        public HourlyForecastPoint(HourlyWeatherModel? data = null, int index = 0, bool isDayTime = true, bool isCurrent = false)
        {
            if (data is null) return;
            Map(data, index, isDayTime, isCurrent);
        }
        ...
        /// <param name="isDayTime">if set to <c>true</c> [is day time].</param>
        /// <param name="isCurrent">if set to <c>true</c> [is current].</param>
        private void Map(HourlyWeatherModel? data, int index, bool isDayTime = true, bool isCurrent = false)
        {
            if (data is null) return;
